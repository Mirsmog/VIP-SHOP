...
class UserController {
  ...
  async login(req, res, next) {
    ...
    if (!user) {
      return next(ApiError.internal("the user doesn't exists"));
    }
    let comparePassword = bcrypt.compareSync(password, user.password);
    if (!comparePassword) {
      return next(ApiError.internal('the password incorrect'));
    }
    const toket = generateJwt(user.id, user.email, user.role)
  }
